...
def create_add_ext_files_log(failed_added_ext_to_files: dict[str, int],
    ...
    with open(add_filext_path, "w") as f:
        ...
        unsuccessful_added_ext_to_ext_msg = f'Unsuccessfully added extension to file(s) with another extension {len(failed_added_ext_to_files)}:'
        unsuccessful_added_ext_to_ext_msg_len = len(unsuccessful_added_ext_to_ext_msg)

        successful_added_ext_to_ext_msg = f'Successfully added extension to file(s) with another extension {len(success_add_ext_to_files)}:'
        successful_added_ext_to_ext_msg_len = len(successful_added_ext_to_ext_msg)

        # write list of failed extension added to file
        if failed_added_ext_to_files:
            f.write(f'\n{cnst.hash_symbol * (unsuccessful_added_ext_to_ext_msg_len * cnst.four_spaces)}')
            f.write(f"\n{cnst.hash_symbol} {unsuccessful_added_ext_to_ext_msg} {cnst.hash_symbol}")
            f.write(f'\n{cnst.hash_symbol * (unsuccessful_added_ext_to_ext_msg_len * cnst.four_spaces)}\n\n')
        ...
        # write msg is all file extensions added successfully
            ...
            f.write(f'\n{cnst.hash_symbol * (cnst.great_job_add_ext_to_ext_msg_len + cnst.four_spaces)}')
            f.write(f'\n{cnst.hash_symbol} {cnst.great_job_add_ext_to_ext_msg} {cnst.hash_symbol}')
            f.write(f'\n{cnst.hash_symbol * (cnst.great_job_add_ext_to_ext_msg_len + cnst.four_spaces)}')
        ...
        if success_add_ext_to_files:
            f.write(f'\n\n{cnst.hash_symbol * (successful_added_ext_to_ext_msg_len + cnst.four_spaces)}')
            f.write(f"\n{cnst.hash_symbol} {successful_added_ext_to_ext_msg} {cnst.hash_symbol}")
            f.write(f'\n{cnst.hash_symbol * (successful_added_ext_to_ext_msg_len + cnst.four_spaces)}\n\n')
